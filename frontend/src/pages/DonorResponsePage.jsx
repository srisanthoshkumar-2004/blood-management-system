...
export default function DonorResponsePage() {
  ...
  useEffect(() => {
    const checkCurrentStatus = async () => {
      try {
        const res = await axios.get(`/request/check-status?requestId=${requestId}&donorId=${donorId}`);
  ...
  const handleResponse = async (responseType) => {
    ...
    try {
      await axios.post(`/request/respond?requestId=${requestId}&donorId=${donorId}&responseStatus=${responseType}`);
