...
export default function Dashboard() {
  ...
  const fetchManagedRequests = async () => {
    try {
      const res = await axios.get('/api/request/managed');
  ...
  const fetchHealthData = async () => {
    try {
      const res = await axios.get(`/api/health/${userId}`);
  ...
  const handleUpdateStatus = async (available) => {
    try {
      await axios.put(`/api/users/${userId}/availability?available=${available}`);
  ...
  const handleHealthSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`/api/health/update?userId=${userId}`, payload);
